...
import { useUIStore, useCartStore, useWishlistStore, useRecentStore, type QuickViewProduct } from '../../_lib/store';
...
function ReviewForm({ productId, productName }: { productId: string; productName: string }) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: { productId: string; author: string; rating: number; title: string; comment: string }) => {
...
function QuickViewContent({ product, onClose }: { product: QuickViewProduct; onClose: () => void }) {
  ...
  const imageList = useMemo(() => {
    ...
    if (images.length === 0) {
      // Generate placeholder variations from the main image
      const base = product.image;
      const separator = base.includes('?') ? '&' : '?';
      images = [
        base,
        `${base}${separator}w=600`,
        `${base}${separator}w=600&h=600&fit=crop`,
        `${base}${separator}w=600&h=800&fit=crop`,
      ];
    }
    return images;
  ...
  }, [product.id, product.name, product.price, product.comparePrice, product.image, product.category.name, addRecent]);
  ...
  const handleAddToCart = () => {
    ...
    toast.success(t('homepage.cart.itemAdded', { name: product.name }), {
      description: `${t('homepage.quickView.quantity')}: ${quantity} × $${product.price.toFixed(2)}`,
  ...
  const tabs = [
    ...
    { key: 'reviews' as const, label: t('homepage.quickView.reviews', { count: reviewCount || product.reviewCount }) },
    ...
    <div className="flex flex-col md:grid md:grid-cols-[55%_45%]">
          ...
          <div className="absolute top-4 end-12 flex flex-col gap-1.5 z-10">
            ...
            {product.stock > 0 && product.stock < 10 && (
              <Badge className="bg-ecommerce-amber text-ecommerce-text-primary border-0 text-xs font-bold px-2.5 py-1 rounded-lg shadow-sm">
                {t('homepage.common.onlyLeft', { count: product.stock })}
              </Badge>
            )}
          </div>
          ...
          <div className="flex gap-2 mt-3 px-1 overflow-x-auto pb-1 scrollbar-hide">
            {imageList.map((img, i) => (
              <button
                ...
                className={`w-16 h-16 rounded-lg overflow-hidden border-2 cursor-pointer transition-all shrink-0 ${
      ...
      <div className="p-6 flex flex-col overflow-y-auto max-h-[70vh]">
          ...
          <div className="flex items-center gap-0.5">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star
                key={i}
                size={14}
                className={i < Math.floor(product.rating) ? 'fill-ecommerce-amber text-ecommerce-amber' : 'text-ecommerce-border'}
              />
            ))}
          </div>
        ...
        <div className="flex items-baseline gap-3 mt-4">
          <span className="text-3xl font-bold text-ecommerce-text-primary">${product.price.toFixed(2)}</span>
          {product.comparePrice && (
            <>
              ...
              <Badge className="bg-ecommerce-emerald/10 text-ecommerce-emerald border-0 text-xs font-semibold">
                {t('homepage.cart.savings')} ${(product.comparePrice - product.price).toFixed(2)}
              </Badge>
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 mt-4">
          {product.stock > 0 ? (
            <>
              ...
              {product.stock < 20 && (
                <span className="text-xs text-ecommerce-amber font-medium">— {t('homepage.common.onlyLeft', { count: product.stock })}</span>
              )}
            </>
          ) : (
        ...
        <div className="flex items-center gap-4 mt-5">
          ...
          <span className="text-sm text-ecommerce-text-muted">
            {t('homepage.cart.total')}: <span className="font-bold text-ecommerce-text-primary">${(product.price * quantity).toFixed(2)}</span>
          </span>
        </div>
          ...
          <div className="flex gap-4 mb-4 border-b border-ecommerce-border/50">
            {tabs.map((tab) => (
              <button
                ...
                className={`text-sm font-medium pb-2.5 border-b-2 transition-all duration-200 flex items-center gap-1.5 -mb-px ${
                  activeTab === tab.key
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === 'description' ? (
                <p className="text-sm text-ecommerce-text-secondary leading-relaxed">{product.description}</p>
              ) : activeTab === 'reviews' ? (
