...
const AddOrEditLink = ({ row, linkSection, data, setData, isNew, open, setOpen, refetch }: AddOrEditLinkProps) => {
  ...
  const [link, setLink] = useState<LinkModel | undefined>();
  const [notify, setNotify] = useState<NotifyProps>({ open: false });

  const loadLink = () => {
    row && linkService.getLinkById(row?.original.id).then((result) => {
      setLink(result.data);
    });
  };
  const onClose = () => {
  ...
  const handleSubmit = (link: LinkModel, setErrors: (errors: FormikErrors<LinkModel>) => void, setSubmitting: (open: boolean) => void) => {
    setSubmitting(true);
    if (isNew == true) {
      linkService
        .addLink(link)
      ...
      linkService
        ...
        .then((result) => {
          if (data && linkSection && result.succeeded) {
            ...
            data[index].imagePreview = link.imagePreview;
            setData([...data]);
            linkSection.original.links = [...data];
          }
          setLink(undefined);
