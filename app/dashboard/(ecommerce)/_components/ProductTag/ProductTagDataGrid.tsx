  ...
  const columns = useMemo<MRT_Column<ProductTagModel>[]>(
    () => [
      ...
      },
      {
        ...
        header: t('fields.productTag.products'),
        enableClickToCopy: true,
        type: 'number'
      }
    ],
  ...
  const DeleteOrEdit = useCallback(
    ({ row }: { row: MRT_Row<ProductTagModel> }) => (
      <Box sx={{ display: 'flex', gap: '1rem' }}>
        <Tooltip arrow placement="top-start" title={t('buttons.productTag.delete')}>
          <IconButton color="error" onClick={() => handleDeleteRow(row)}>
            <Delete />
          </IconButton>
        </Tooltip>
        <Tooltip arrow placement="top-start" title={t('buttons.productTag.edit')}>
