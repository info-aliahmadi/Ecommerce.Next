...
function CountryDataGrid() {
  ...
  const handleEditRow = (row : MRT_Row<CountryModel>) => {
    let CountryId = row.original.id;
    setIsNew(false);
    setRowId(CountryId);
    setOpen(true);
  };
  const handleDeleteRow = (row: MRT_Row<CountryModel>) => {
    setRow(row);
    setOpenDelete(true);
  };
  const handleRefetch = () => {
  ...
  const handleCountryList = useCallback(async (filters : GridDataBound) => {
    return await service.getCountryList(filters);
  }, []);
