...
const AddOrEditCountry = ({ countryId, isNew, open, setOpen, refetch }: { countryId: number, isNew: boolean, open: boolean, setOpen: (open: boolean) => void, refetch: () => void }) => {
  ...
  const handleSubmit = (Country, setErrors: (errors: FormikErrors<LinkModel>) => void, setSubmitting: (open: boolean) => void) => {
    if (isNew == true) {
      countryService
        .addCountry(Country)
        .then(() => {
