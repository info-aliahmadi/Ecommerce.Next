...
import {
  ...
  Undo, Redo, Sun, Moon, Image as ImageIcon,
  AlignLeft, AlignCenter, AlignRight, Upload,
  Link, Unlink, Minus, Code, Terminal, Table as TableIcon,
  FileCode, Eye, Pencil, Command, Type, Quote, Indent, Outdent
} from "lucide-react";
...
function EditorContent({
  ...
  const exportHTMLSafely = React.useCallback(() => {
    if (!editor) return "";
    ...
    try {
      ...
      return editor?.getRootElement()?.innerHTML ?? "";
    }
  }, [editor]);

  const syncHtmlValue = React.useCallback((html: string) => {
    if (lastSyncedHtmlRef.current === html) return;
  ...
  const methods = useMemo<DefaultTemplateRef>(
    () => ({
      ...
      getHTML: () => modeRef.current === "html" ? contentRef.current.html : exportHTMLSafely(),
    }),
    [editor, exportHTMLSafely, syncHtmlValue],
  );
  ...
  useEffect(() => {
    ...
    const unregisterHtmlSync = editor.registerUpdateListener(() => {
      if (!onHtmlChangeRef.current) return;

      setTimeout(async () => {
        const html = exportHTMLSafely();

        setContent((prev) => prev.html === html ? prev : { ...prev, html });
        syncHtmlValue(html);
      }, 500);
  ...
  }, [editor, commands, onReady, methods, exportHTMLSafely, syncHtmlValue]);
  ...
  const handleMarkdownChange = (markdown: string) => {
    ...
    if (!hasExtension("markdown")) return;

    if (markdownSyncTimeoutRef.current) {
      clearTimeout(markdownSyncTimeoutRef.current);
    }

    markdownSyncTimeoutRef.current = setTimeout(async () => {
      await commands.importFromMarkdown(markdown, { immediate: true, preventFocus: true });
    ...
    <>
      ...
      <div className="lexkit-editor">
        <div className="flex flex-col flex-1" style={{ display: mode === "visual" ? "flex" : "none" }}>
          <RichTextPlugin
            contentEditable={<ContentEditable className={dir == "rtl" ? "lexkit-content-editable-rtl" : "lexkit-content-editable"} />}
            placeholder={<div className={dir == "rtl" ? "lexkit-placeholder-rtl" : "lexkit-placeholder"}>{placeholder}</div>}
